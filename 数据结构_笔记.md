...
线性表是 **逻辑结构**  ，是具有 **相同数据类型** 的n个数据元素的有限序列，n=0时为空表。
...
常见的遍历：先序(NLR)、中序(LNR)、后续(LRN)、层次遍历
...
void PostOrder(BiTree T){
    ...
    while(p || !IsEmpty(s)){
        ...
        if(p){
            ...
            p = s.top();    // p设置为栈顶结点
            if(p->lchild && p->rchild != r) // 若右孩子存在, 且未被访问过
                p=p->rchild;
...
4. 二叉树在线索化后仍不能有效求解的问题是 **后序线索二叉树中求后续后继**

5. **后序线索树** 的遍历需要栈的支持

6. 【2011】一棵二叉树的前序和后序分别为1234和4321，则中序不可能是

7. 【2015】先序序列为abcd的不同二叉树的个数为

8. 【878，2019】某二叉树的中序序列和后序序列正好相反，则该二叉树一定是 **任一结点无左孩子**

9. 【878，2020】某非空二叉树的先序遍历和后序遍历序列正好相反，则该二叉树一定是 **无法确定**
...
2. 设X是数T中的一个非根结点，B是T对应的二叉树。在B中，X是其双亲结点的有孩子，下列正确的是 **在树T中，X一定有左边兄弟**
...
- 再散列法：
    ...
    冲突的时候用另一个hash函数算增量d **最多m-1次就会遍历表中的全部位置**
- 伪随机法
...
注意：目前来看，线性探测再散列就是线性探测法，二次探测再散列就是平方探测法
