...
是数据的基本单位，通常作为一个整体，一个数据元素由若干个（可以是不同类型的） **数据项（最小不可分割）** 组成， **数据对象** 是有相同性质的数据元素的集合
...
- `线性结构（只存在一对一的关系）` ：线性表、受限的线性表（栈、队列、串）、线性表推广（数组）
...
常见的逻辑结构：线性、队列、栈、二叉树、堆、图

常见的物理结构：链表、**循环队列（用顺序表表示的队列）** 、链栈、顺序栈、线索树
...
- 两种不同的数据结构，逻辑结构或物理结构一定不相同吗？

    例如二叉树和BST，逻辑结构和存储结构完全相同，但对于运算的定义有不同。

    比如查找。二叉树的时间复杂度为O(n)，BST的时间复杂度为O(logn)

- 举例，相同逻辑结构，不同存储方式实现下效率不同

    逻辑结构线性表，顺序表情况下插入删除需要O(n)，链表情况下插入删除只要O(1)
...
1. (logN)^2^是O(N)的，因为(logN)^2^比N小
2. 递归求斐波那契数列的时间和空间复杂度都是 **O(Fn)**
3. 用渐进表示法分析算法复杂度的增长趋势
...
1. 找到运行次数最多的语句，然后找到运行次数t和循环终止变量的关系
2. 确定循环停止的条件并求解
...
- 上面的时间复杂度为O(N4)，下面的时间复杂度为O(N2)，所以总时间复杂度为O(N4)
...
要判断一个整数N（>10）是否素数，我们需要检查3到根号N之间是否存在奇数可以整除N，时间复杂度是O(根号N)
...
下给出四种算法的时间复杂度，如果当 N=100 时它们对应的运行时间依次是：A 算法 100 毫秒、B 算法 30 毫秒、C 算法 20 毫秒、D 算法 10 毫秒，则当 N=200 时，哪种算法最快？ **B**
...
1. 算法与计算机无关，但是与具体的程序设计语言有一定的相关性。

2. 存储结构与数据取值无关。

3. 《数据结构》研究非数值计算的程序设计问题中计算机的操作对象以及它们之间的关系和操作等的学科。

4. 抽象数据类型(ADT)是指一个数学模型及定义在该模型上的一组操作。抽象数据类型的定义仅取决于它的一组逻辑特性，而与其在计算机内部如何表示和实现无关。

5. 数据的逻辑结构与数据元素本身的内容和形式无关，是数据元素之间的逻辑关系。

6. 在数据结构中，从逻辑上可以把数据结构分成（ 线性结构和非线性结构 ）

7. 求n的阶乘的递归算法的复杂度是O(n)
...
线性表是 **逻辑结构**  ，是具有 **相同数据类型** 的n个数据元素的 **有限** 序列（序列说明有前后关系），n=0时为空表。

**顺序表** 、 **链表** （包括邻接表）是 **存储结构** ，是线性表的顺序实现和链式实现
...
1. 若线性表常用操作是存取第i个元素及其前驱和后继元素的值，为了提高效率，应采用： **顺序表**
   ...
   ```
   错选双向链表  顺序表取第i、i-1、i+1的元素都是O(1)。双向链表找第i个元素时还是得从头依次查找，需要O(n)
   ```
...
3. 【2011】一个长度为L（L >= 1）的升序序列的第 [L / 2]上取整称为中位数。
...
8. 将长度分别为m,n的两个单链表合并为一个单链表的时间复杂度为 O(max(m,n))
...
9. 循环链表和双向循环链表是存储结构 **正确**

    ```
    线性表是逻辑结构, 线性和非线性是从逻辑结构来划分的
    循环链表和双向循环链表与线性表属于不同层次，是线性表的特殊存储方式
    ```
...
5. 原地实现单链表逆置
    ...
    */
    Linklist reverse_2(Linklist L){
        // p指向当前结点 pre指向p的前驱 r指向p的后继结点，防止断链
...
二叉树每个结点至多只有两颗子树，且子树有左右之分，是 **有序树** 。
...
1. **顺序存储结构**
    ...
    数组（像静态链表），下标反映结点之间逻辑关系（下标起点是1不是0）
...
6. 【2020 408】对于任意一棵高度为5且有10个结点的二叉树，若采用顺序存储，每个结点占1个存储单元（仅存放结点的数据），则存放该二叉树需要的存储单元数量至少是 ==31==
...
7. 【878，2016】设一棵非空完全二叉树 T 的所有叶节点均位于同一层，且每个非叶结点都有 2 个子结点。若 T 有 1024 个叶结点，则 T 的结点总数是 ==2047==
...
8. 若将一棵树 T 转化为对应的二叉树 BT，则下列对 BT 的遍历中，其遍历序列与 T 的后根遍历序列相同的是： ==中序==
...
9. 对于一个有N个结点、K条边的森林，共有几棵树？ ==N-K==

    ```
    由一颗树的性质：结点等于边+1, 即n= k+ 1
    也就是说一棵树顶点会比边多1
    现在多了n-k，说明有n-k棵树
    ```
...
10. 具有1102个结点的完全二叉树一定有 ==551== 个叶子结点

    ```
    完全二叉树, 度为1的节点最多有1个，可以没有
    于是n2+n1+n0 = 1102, 又有n0=n2+1
    1101 = 2*n2+n1;
...
11. 由若干个二叉树组成的森林F中，叶结点总个数为N，度为2的结点总个数为M，则该集合中二叉树的个数为 ==N-M==

     ```
     一个二叉树中n0=n2+1
     现在整个森林中n0比n2多了N-M, 所以有N-M棵树
     ```
...
1. 已知一颗二叉树按顺序存储结构进行存储，设计一个算法，求编号为i和j的 **两个结点的最近的公共祖先** 的值
    ...
    ```c++
    /*
    任意两个结点一定有公共祖先（最坏情况下是根结点）
    每次都将大的那个除2，来到他的祖先，最终ij一定能重合
    假设数组下标从1开始
    */
    
    // 查找结点i和j的最近公共祖先的结点的值
    int get_comm_ancestor(SqTree T, int i, int j){
...
常见的遍历：先序(NLR)、中序(LNR)、后序(LRN)、层次遍历

任何一棵二叉树的叶结点在先、中、后遍历序列中的相对次序不发生改变
...
对于前三种遍历来说， **遍历的路线都是一样的** （前三种都是DFS，层次是BFS）

都是从根结点出发，沿左子树走到最左，无法继续深入时，退回上一结点，然后进入右子树，重复上述过程，直到回到根结点为止

![第五章_二叉树的遍历路线](assets/数据结构_笔记/第五章_二叉树的遍历路线.png)

三个遍历的区别在于， **访问结点的时机不同**

- **先序遍历** ： **遇到结点就访问**
- **中序遍历** ： **从左子树返回时访问**
- **后序遍历** ： **从右子树返回时访问** ，（即左右孩子都已经访问过了，才访问）
...
- **时间复杂度：**  **O(n)** ，每个结点都恰好被访问一次
- **空间复杂度：**  **O(n)** ，在递归过程中，递归工作栈的深度恰好为树的高度，最坏情况下树高为n
...
void PostOrder(BiTree T){
    ...
    while(p || !IsEmpty(s)){
        ...
        if(p){
            ...
            p = s.top();    // p设置为栈顶结点
            if(p->rchild && p->rchild != r) // 若右孩子存在, 且未被访问过
                p=p->rchild;
            
            // 此时 不存在右孩子或者能确定左右孩子都已经被访问 可以访问当前结点了
            else{
                s.pop();    // 栈顶出栈
                visit(p->data); // 访问当前结点
                r = p;  // 记录最近访问的结点
...
![第五章_后序和中序建树](assets/数据结构_笔记/第五章_后序和中序建树.png)

建树必须要有中序

后序遍历中根总是最后出现的，可以在中序序列中找到根所在的位置，此时左右两边的元素就各自属于左右子树

在左右子树上重复上述过程，每次找到当前小团体的根，并从中序序列中得到它的左右子树
...
- **后序线索树** 的遍历仍需要栈的支持
- 二叉树在线索化后仍不能有效求解的问题是 **后序线索二叉树中求后续后继**
...
1. 在二叉树中有两个结点m和n，若m是n的祖先，则 ==后序遍历== 可以找到m到n的路径

    ```
    在后序遍历退回时访问根结点，就可以从下向上吧从n到m的路径上的结点输出
    若采用非递归算法，当后序遍历访问到n时，栈中把从根到n的父指针的路径上的结点都记忆下来，也可以找到m到n的路径
    ```
...
2. 线索二叉树是一种 ==物理结构==
    ...
    ```
    二叉树是逻辑结构，线索化后是链表结构，是二叉树的链式存储，是物理结构
    ```

    

3. 一棵左子树为空的二叉树在先序线索化后，其中空的链域的个数是 ==2==

    ```
    根的前驱指针和最有一个元素的后继指针为空
    ```

4. 【2011】一棵二叉树的前序和后序分别为1234和4321，则中序不可能是 ==3,2,4,1==

    ```
    前序和后序刚好相反，说明每个结点最多只有一个孩子
    分析可得1为根结点，2为根节点的唯一孩子
    那么中序序列中去掉根结点的1之后，2应该在最前或者最后，通过排除法得到
    ```

5. 【2012】若一棵二叉树前序：aebdc，后序：bcdea，则根结点的孩子为：==e==

    ```
    虽然前+后不能唯一确定一棵树
    但如果两个结点前序XY，后序YX，则可说明X是Y的祖先
    由题意可知a是根结点，e是a的孩子
    又可得到e是bdc的祖先
    所以a的孩子只有e
    ```
...
6. 【2015】先序序列为abcd的不同二叉树的个数为 ==14==

    ```
    由前序和中序的递归算法中栈的变化可知：前序+中序相当于以前序序列入栈，以中序序列出栈
    因此前+中 唯一确定一棵树
    因此题目相当于“入栈abcd，有多少出栈序列”
    用n=4的卡特兰数计算
    ```

    

7. 【878，2019】某二叉树的中序序列和后序序列正好相反，则该二叉树一定是 ==任一结点无左孩子==

8. 【878，2020】某非空二叉树的先序遍历和后序遍历序列正好相反，则该二叉树一定是 ==无法确定==

    ```
    相反: 可以得出结论左子树或者右子树为空，且树的高度等于结点个数
    相同: 只有可能为只有根节点
    ```
...
1. 假设二叉树采用二叉链表存储结构，设计一个 **非递归算法求二叉树的高度**

    ```c++
    /*
    树高==层数
    采用层次遍历 遍历的过程中用level来记录层数
    关键在于如何判断到下一层：
        用变量last来记录当前层的最右结点
        当队头下标==最右结点时, 本层遍历完, 此时队尾就是下一层的最右结点
    */
    ...
    int get_depth(BiTree T){
        ...
        // last指向当前层的最右结点 level为层数
        int last = 0, level = 0;
        
        // 队列 初始将根结点入队 p表示正在访问的结点
        Queue Q;
        Q[rear++] = T;
        BiTree p;
    ...
    int get_depth(BiTree T){
        ...
        // 左右子树的高度
        l_depth = get_depth(T->lchild);
        r_depth = get_depth(T->rchild);
        
        if(l_depth > r_depth)   // 此时树的高度为左子树+根结点
            return l_depth + 1;
        else
            return r_depth + 1;
    }
...
2. 【重点】先序和中序遍历建树
    ...
    ![前序和中序建树](assets/数据结构_笔记/前序和中序建树.png)
    ...
    先序+中序 构建二叉树 的伪代码
    递归实现
    每次找到当前区间的根结点, 并将其他元素区分左右
    */
    
    先序中序建树(先序序列pre, 中序序列in, 先序的左端点pre_l, 先序的右端点pre_r, 中序的左端点in_l, 中序的右端点in_r){
        
        建立一个空节点root, 赋值为pre[pre_l]; // 先序的第一个结点就是整个树的根
        
        找到根结点在中序序列的下标; // 此时能将区分左右子树
        
        得到左子树长度和右子树长度;
        
        root->lchild = 根据左子树在两个遍历中的区间递归建树;
        root->rchild = 根据左子树在两个遍历中的区间递归建树;
...
3. 二叉树采用二叉链表形式存储，设计算法判断该二叉树是否是完全二叉树

    ```c++
    /*
    判断树是否是完全二叉树 完全二叉树和满二叉数中1~n的结点一一对应
    因此层次遍历(能实现1~n顺序访问)中, 空结点后还有非空结点的, 一定不是完全二叉树
...
4. 二叉树采用二叉链表形式存储，设计算法将树中所有结点的左右子树互换
...
5. 二叉树采用二叉链表形式存储，设计算法求先序遍历序列中第k个结点的值
    ...
    void get_k(BiTree T, int k) {
    	if(T){
    		...
    		if (num == k){
    		    ...
    		    int ans;
    			if(T->lchild)
    			    ans = get_k(T->lchild,k);
    			if(T->rchild)
    			    ans = get_k(T->rchild,k);
    		}
    	}
    	return ans;
    }
...
6. 二叉树采用二叉链表形式存储，设计算法删除以值为x的结点为根的子树，并释放空间

    ```c++
    /*
    层序遍历整个二叉树，中间遇到值为x的结点就删除
    */
    
    // 删除以值为x的结点为根的子树
    void del_x(BiTree &t){
        // 删除左右子树(因为左右子树的空间也要释放), 释放空间
        if(t){
            del_x(t->lchild);
            del_x(t->rchild);
            free(t);
...
7. 在二叉树中查找值为x的结点，设计算法（用C）打印值为x的结点的所有祖先，结点值为x的结点不多于一个

    ```c++
    /*
    找值为x的结点的所有祖先, 题中明确了x的值不多于一个
    
    采用非递归后序遍历
    访问到值为x的结点时, 栈中所有元素均为x的祖先
    依次打印出栈即可
    */
    ...
    void PostOrder(BiTree T, int x){
    	InitStack(S); // 初始化栈
    	BiTree p = T; // p为遍历指针
    	BiTree r = NULL; // r指向最近访问过的结点 也可以增加一个标志域tag记录是否已被访问
    	...
    	while(p || !StackEmpty(S)){ 
    		...
    		// 向右
    			...
    			//弹出结点
    				...
    				pop(S, p);
    				r = p;  // 记录最近访问过的结点
    				p = NULL; // 结点访问完毕后重置p指针 不然该结点又会重新入栈
    			}
    		}
    	}
    }
...
8. 设一棵二叉树的结点结构为 `(LLINK,INFO,RLINK)` ROOT为指向该二叉树根结点的指针，p和q分别指向该二叉树中任意两个结点的指针，编写算法找p和q的最近公共祖先

    ```c++
    /*
    采用非递归后序遍历时，栈中存放二叉树的结点
    当访问到某点时, 此时栈中的结点均是该结点的祖先
    */
    ```

    

9. 假设二叉树采用二叉链表存储，设计一个算法求非空二叉树b的 **宽度**

10. 设有一棵满二叉树（所有结点值均不同），已知先序序列pre，求后序序列post

11. 设计一个算法将二叉树的叶结点按从左到右的顺序连成一个单链表，表头指针head。二叉树采用二叉链表存储，链接时用叶结点的右指针域来存放单链表指针

12. 设计一个算法判断两棵二叉树 **是否相似**

13. 写出中序线索二叉树里查找指定结点在后序的前驱结点的算法
...
- **先根遍历** ： 和该树对应的二叉树的 **先序** 相同
- **后根遍历** ： 和该树对应的二叉树的 **中序** 相同
...
1. 设F是一个森林，B是F变换来的二叉树，若F中有n个非终端结点，则B中右指针域为空的结点有 ==n+1== 个

    ```
    森林转化的二叉树，右指针域为空代表他没有兄弟
    首先转化为二叉树后，最后一棵树的根结点右指针显然为空	这是1
    其次对于每个非终端结点，他最后一个孩子的右指针也为空	这是n
    因此就是n+1
    ```

    

2. 设X是数T中的一个非根结点，B是T对应的二叉树。在B中，X是其双亲结点的右孩子，下列正确的是 ==在树T中，X一定有左边兄弟==

    ```
    记双亲结点为Y
    B中x是双亲的右孩子，则在T中x是Y的右兄弟
    也就是X一定有左兄弟
    ```

    

3. 【2011】已知一棵有2011个结点的树，其叶结点有116，该树对应的二叉树中无右孩子的结点个数是 ==1896==

    ```
    可以看看上面的1
    转化为二叉树后，
    根结点没有右孩子 这是1
    其次每个非终端结点的最右孩子转化后也没有右孩子 这是n
    因此答案就应该是：非终端结点数+1
    也就是2011-116+1 = 1890
    ```
...
4. 【2014】将森林F转换为对应的二叉树T，F中叶结点个数等于 ==T中左孩子指针为空的结点个数==

    ```
    森林的叶结点没有孩子，转化为二叉树后他就没有左结点
    ```

    

5. 【2016】若森林F有15条边、25个结点，则F包含的树的个数是 ==10==
...
6. 【2020】已知森林F及其对应的二叉树T，若F的先根遍历序列是abcdef，中根遍历序列是badfec，则后根遍历序列为 ==bfedca==
...
每次选择最小的两个结点作为新结点的左右孩子，新结点的权值为左右孩子权值之和，最终形成一棵树
...
```c++
/*
因为构造哈夫曼树需要找当前最小的两个结点 因此用到了小顶堆及其删除、插入操作
此处堆中的元素是加了权值的树结点的指针
*/

// HuffmanTree为指向结点的指针类型
...
// 哈夫曼树的构建 最小堆中的元素类型是HuffmanTree
HuffmanTree Huffman(MinHeap H){
    HuffmanTree T;  // 当前结点
    
    BuildHeap(H);   // 将H调整为小顶堆
    
    // 需要n-1次合并
    for(int i = 1; i < H.size(); i++ ){
        T = (HuffmanTree)malloc(sizeof(struct HTNode)); // 建立一个新结点
        
        // 两次取出最小的元素, 作为新结点的左右孩子 并计算权值
        T->l = DeleteMin(H);
        T->r = DeleteMin(H);
...
如果有若干个权值不同的结点，那么得到的 **哈夫曼树不唯一，哈夫曼编码也不唯一** 

但是WPL必定相同，且都是最小的

此外，任何一个字符的编码都不会是另一个字符编码的前缀，哈夫曼编码也称为 **前缀编码**
...
堆也称为 **优先队列** ，从堆中取元素的顺序是按元素的 **优先级** 大小，而不是元素进入堆的次序

通常用一棵 **完全二叉树** 来表示一个堆，并且用 **数组（下标从1开始）** 来实现堆的存储

- `大顶堆` ： 是一棵完全二叉树，每个节点的值不小于其孩子结点的值，此时是最大值优先
- `小顶堆` ： 
- 注意： **兄弟结点之间并没有大小关系** ，但每个结点和他的父结点或孩子结点存在大小关系
...
- `int DeleteMax(MaxHeap H)` 删除并返回堆中的最大元素
...
```c++
// 堆结构的定义
typedef struct HNode* Heap; // Heapd代表HNode*类型
struct HNode{
    int* data;  // 存储元素的数组
    int size;   // 堆中元素个数
    int capacity;   // 最大容量
...
MaxHeap CreateHeap(int MaxSize){
    ...
    // data为长度为MaxSize+1的int*型数组 因为下标从1开始
    H->data = (int*)malloc((Maxsize + 1)) * sizeof(int);

    H->size = 0;
    H->capacity = MaxSize;
    H->data[0] = MAX;   //一个大于所有可能元素的值 作为哨兵
...
**堆的向上调整算法（通过插入操作建立最大堆）:**

1. 插入到叶结点，并向上调整，此时是从该结点开始逐渐向上进行调整
2. 重复n次插入，调整的过程最终建立最大堆（虽然下图是最小堆，但我想你已懂了它的内涵）

![第五章_堆的向上调整算法](assets/数据结构_笔记/第五章_堆的向上调整算法.png)

时间复杂度为 **O(nlogn)** ， 一次插入并调整的时间复杂度是O(logn)，重复n次
...
**堆的调整算法（在线性时间建立最大堆： Floyd 建堆算法）：**

建立最大堆

1. 将元素按输入顺序存放在二叉树中
2. 从最后一个有孩子的结点开始，逐一进行向下调整操作，直到根结点也完成
...
- 空间复杂度：O(1)，常数个临时变量
- 时间复杂度：**O(n)** ，光从代码上看，觉得是O(nlogn)，有空再展开细说-=
...
这个图给的是最小堆，那么根结点选择和孩子中最小的结点进行交换

![第五章_堆的向下调整算法](assets/数据结构_笔记/第五章_堆的向下调整算法.png)
...
1. 【878，2016】设最大堆的层序遍历结果为 { 95, 72, 86, 50, 65, 12, 23, 42 }。用线性时间复杂度的算法将该堆调整为最小堆。则该树的中序遍历结果为：

    ```
    72, 50, 42, 65, 12, 86, 23, 95
    ```

    - 最大堆就是个banana，调整为最小堆的时候当成普通二叉树即可。然后从最后一个有孩子的结点开始调整，一路向左

    

2. 【878，2019】在一个有 2333 个元素的最小堆中，下列哪个下标不可能是唯一最大元的位置？
    ...
    - 小根堆中最大的数一定是放在叶子节点上，堆本身是个完全二叉树，完全二叉树的叶子节点的位置大于[n/2]，也就是说一定要大于1166

    

3. 【878，2020】将文本 `this is a test` 进行哈夫曼编码后，该文本所占总位数为 **38**

    空格也算字符，不要忘了

    ![第五章_习题3](assets/数据结构_笔记/第五章_习题3.PNG)

4. 若度为m的哈夫曼树中，叶结点的个数为n，则非叶结点的个数为：

    ```
    一般说的哈夫曼树指的是最优二叉树，但哈夫曼树不完全局限于二叉树，度为m的二叉树就是最优m叉树
    度为m的二叉树中只有度为m和度为0的结点，非叶结点的个数记作x
    由总结点数=分枝数+1=所有结点度数和+1
    x+n = m*x + 1，从中解出x，然后要上取整
    ```
...
- 无向图：边： `(v, w)`
- 完全图：图中任意两个顶点之间都 **有边** 。类似的：有向完全图
- 子图：顶点集合边集都是原来图的子集。如果含原来图的全部顶点就是生成子图。注：并非任何子集都能组成子图
- 连通： **有路径存在** 。类似的：强连通（有向图来去都有路）
- 连通图：无向图中任意两个顶点都是连通的 **有路径**
- 连通分量：无向图中的 **极大** (再多一个顶点就会非连通) **连通** **子图**
- 强连通图：有向图中任意两个顶点都是强连通的
-  **生成树** ：包含图全部顶点的极小连通子图（一定包含n个顶点和n-1条边）
- 生成森林：非连通图连通分量的生成树构成
...
3. 一个有28条边的非连通无向图至少有顶点 ==9==
    ...
    ```
    考虑非连通图最极端的情况：由一个完全图和一个独立的顶点组成，此时多一条边就是连通图
    28 = n(n-1)/2，得到n = 8。再加上一个独立顶点得到9
    ```
...
4. 若一个具有n个顶点、e条边的无向图是一个森林。则森林中必有 ==n-e== 棵树
    ...
    ```
    n个顶点的树有n-1条边。假设有x棵树组成森林，可以通过增加1个结点和x条边使森林组成一棵树
    此时树有n+1个结点，e+x条边
    列方程n+1 = e + x + 1 ，解得 x = n - e
    ```
...
5. 【2010】若无向图G=(V,E)中含7个顶点，要保证图G在任何情况下都是连通的，最少需要的边数 ==16== 
    ...
    ```
    首先先让6个结点组成一个完全图，再加上一条边就必定能保证另一个结点也连通
    6个结点的完全图需要 6*(6-1)/2=15，再加上一条边得到16
    ```

    

6. 【2017】已知无向图G有16条边 ，其中度为4的顶点个数为3，度为3的顶点个数为4，其他顶点的度均小于3。图G所含顶点个数 **至少** 为 ==11==

    ```
    至少的话，其余顶点都是度为2的顶点
    无向图各顶点的度数之和 = 边数的2倍
    列出方程：4*3+3*4+2*x = 16*2，得其他顶点为4。因此总顶点数4+4+3=11
    ```

    

7. 【878，2017】在 *N* 个顶点的无向图中，所有顶点的度之和不会超过顶点数的多少倍？==N−1==

    ```
    完全图的时候顶点度数之和才能最大, 为N*（N-1）, 除以顶点数n得到答案
    ```
...
- 1是边，0不是边
- 网/带权图：权Wij表示有边，0或∞表示不存在边

![第六章_邻接矩阵](assets/数据结构_笔记/第六章_邻接矩阵.png)
...
- **空间复杂度** ：**O(V^2)**  
- **适合** ： **稠密图** （边多）
- 设图G的邻接矩阵A，Aij的k次方的含义： **从i到j的长度为k的路径的数目** 
...
- 顶点表结点包括：`顶点数据域data` 和 `指向第一个邻接点的指针域firstarc`

- 边表结点但包括：`邻接点域adjvex` 和 `指向下一个邻接点的指针域nextarc`

![第六章_邻接表](assets/数据结构_笔记/第六章_邻接表.png)
...
- 对于有向图，邻接表可以方便得到结点的出度，当需要方便得到结点的入度时，可采用 **逆邻接表** （对每个顶点vi建立一个连接所以进入vi的边的表）
- **优点** ： 便于增加和删除顶点；便于统计边的数目，时间复杂度是O(n+e)；空间效率高
- **缺点** ： 不便于判断顶点之间是否有边；不便于计算有向图各顶点的度
- **空间复杂度** ：**O(V + E)** 
- **适合** ： **稀疏图**
...
- 顶点结点：数据data，指向第一个入弧结点的指针firstin，指向第一个出弧结点的指针firstout
- 弧结点：弧尾tailvex，弧头headvex，指向下一个入弧结点的指针，指向下一个出弧结点的指针，自身的信息info



![第六章_十字链表](assets/数据结构_笔记/第六章_十字链表.PNG)
...
- 容易计算结点的出度和入度
- 十字链表不唯一（因为第一条、下一条的顺序都可以改），但十字链表确定一个图
...
![第六章_临接多重表](assets/数据结构_笔记/第六章_临接多重表.PNG)

**重点：**

- 同一条边在邻接表中用两个结点表示，而在邻接多重表中只有一个结点（更方便删除）
...
1. 有向图的邻接表存储中，顶点v在边表中出现的次数为 ==顶点v的入度==

    ```
    边表不包含顶点表，有向图下，一条边的起点在顶点表中，终点在边表中。
    ```
...
1. 写出从图的邻接表转换为邻接矩阵的算法

2. 【2015】设邻接矩阵A，A的n次的i行j列的含义是什么
...
3. 【2021】已知无向连通图G由顶点集V和边集E组成，|E|>0，当G中度为奇数的顶点个数为不大于2的偶数时，G存在包含所有边且长度为|E|的路径（称为EL路径）。设图G采用邻接矩阵存储，类型如下：
    ...
    设计算法int IsExistEL(MGraph G) ， 判断G是否存在EL路径，存在返回1，不存在返回0
...
从图的某一结点v出发，访问与v相邻且未被访问的任意结点w，再从w出发……重复过程。

当不能再继续向下访问时，依次退回到最近被访问的结点，若它还有邻接结点未被访问，则从该点开始继续上述搜索过程，直到图中所有结点都被访问为止。
...
**时间复杂度花在查找每个结点的邻接点上** ，因此时间复杂度和数据结构有关
...
- 采用 **邻接矩阵** ：
    - 查找每个结点临界点需要用时O(|V|)，总共有O(|V|)个结点。因此 **时间复杂度O(|V|^2)** 
...
1. 图的广度优先生成树的树高比深度优先生成树的树高 ==小或相等==
   ...
   ```
   对于广度优先生成树，顶点到其他顶点的路径是图中对应的最短路径。树高最小。
   深度优先生成树因为是要深，路径尽可能长，所以树比较高
   ```

   

2. 【878，2017】如果无向图 G 必须进行两次广度优先搜索才能访问其所有顶点，则下列说法中不正确的是：==A==
...
1. 写出图的DFS算法的非递归算法（图采用邻接表形式）
    ...
    /*
    用一个栈S来记录下一步可能访问的顶点
    用访问数组visited[]来记录是否访问, 已经访问过的节点不能再入栈
    图采用邻接表形式
    
    由于使用了栈, 访问方式从右到左,但仍是DFS
    */
...
3. 假设图用邻接表表示，设计一个算法，输出顶点Vi到顶点Vj的所有简单路径

   ```c++
   /*
   采用递归的DFS
   从结点u出发, DFS遍历图中结点, 若访问到v, 则输出搜索路径上的结点
   设置path来保存路径
   d为路径长度
   
   算法思路：
       1. 如果u直接找到v, 打印结束
       2. 若找不到邻接点, 将将visited[u]=0, 并返回
       3. 若找到邻接点w, 则从w开始找v, 否则将visited[w]=0, 并返回
       3. 以此类推, 直到找到v
   */
   
   // 从图G中找到顶点u和v的路径 d为路径长度 path为路径上的结点
...
1. 树形不唯一，但若图G中各边权值互不相等，则最小生成树唯一
2. 当G本身是一棵树（边数  = 顶点数 - 1），最小生成树是它本身
3. 最小生成树的权值和总是唯一的
...
- prim是选择到当前集合最小的边，并将该边和顶点加入集合，直到所有顶点都在集合中
- Kruskal是将多个孤立的森林逐渐合成一棵树的过程
...
类似于 **最短路径的Dijkstra算法** ，区别在于 **prim更新的是到集合的距离，Dijkstra算法更新的是到源点的距离**

**Prim算法的思想：**

1. 初始时任选一顶点加入树T
2. 每次选择一个 **与当前树T中顶点集合距离最近的顶点** （ **或者说选择当前最小边加入集合**  ），将对应顶点和边加入T 
3. 直到图G所有顶点都加入T
...
void Prim(G, T){
    ...
    T = {w};     // 任意加入一个顶点w
    E = {};    // 边集合为空
    while(生成树T中不含图G的全部顶点)：
        找到外面的顶点v到树内顶点u的 权值最小的边(u, v)
        顶点、边加入对应集合
        更新其余顶点到集合的距离
}
```
...
-  **时间复杂度O(|V|^2)** ，和|E|无关，因此 **适合稠密图** ，（ **Prim选的是顶点，边多无所谓！** ）
-  可以改进的地方：
    -  当稀疏图时，改用邻接表
    -  找到当前树中顶点距离最近的顶点可以采用最小堆，改进为 **O(|E|log|v|)**
 ...
 **Prim选择的是顶点，Kruskal选择的是边** ， **按边的权值从小到大** 选择合适的边来构造最小生成树
...
- 使用 **邻接矩阵** 存储时，时间复杂度为 **O(|V|^2)** 

- 使用 **带权的邻接表** 存储时，虽然修改dist[ ]的时间会减少，但在dist[ ]中找到最小分量的时间不变，时间复杂度还是时间复杂度为O(|V|^2)
...
6. 【878，2019】如果从有向图 G（至少有 2 个顶点）的每一点均能通过深度优先搜索遍历到所有其它顶点，那么该图一定不存在拓扑序列 **正确**
...
7. 【878，2017】以下哪个命题是正确的？ **D**

    ```c++
    A. 对于带权无向图 G = (V, E)，M 是 G 的最小生成树，则 M 中任意两点 V1 到 V2 的路径一定是它们之间的最短路	
    B. P 是顶点 S 到 T 的最短路径，如果该图中的所有路径的权值都加 1，P 仍然是 S 到 T 的最短路径
    C. 深度优先遍历也可用于完成拓扑排序
    D. 另外三个选项都不对
    ```

    - A 可能最短路径是权值相同的边,没有在最小生成树中
    - B 假如说最短路径上一共有 10 条边，而另一条路径虽然比最短路径长，但它只有一条边，如果全加 1，就会导致边少的路径成为新的最短路径
    - C 可能因为DFS能直接得到的是逆拓扑排序- =

    

8. 深度优先遍历一个有向无环图，并在算法退栈返回时打印相应的顶点，则输出的顶点序列是 **逆拓扑排序**
    ...
    DFS求拓扑排序的最后一步就是讲数组逆置
    ```

    

9. 【878，2020】在拓扑排序算法中用堆栈和用队列存储 0 入度的结点，产生的结果会不同吗？`有可能不同`

    ```
    比如有现在同时有两个入度为0的结点可供选择，用栈的话会选择后来的那个，对队列的话会选择先来的那个
    都是拓扑排序，但是顺序不同
    ```
...
- 散列查找：构造、冲突处理办法、手动模拟成功和失败的平均查找长度、特征、性能分析（计算装填因子）
...
5. 【2017】下列二叉树中，可能成为折半查找判定树（不含外部结点）的是： **A** 
   ...
   直观来说，如果左子树比右子树多, 那么所有子树都应该满足这个规则
   ```
...
```c++
// 二叉排序树的插入
Bitree BST_Insert(Bitree &t, int k){
    // 原树为空, 新插入的结点作为根结点
    if(t == NULL){
...
3. 如果结点z有两棵子树，则令z的直接后继（也就是比目标大的最小结点）或直接前驱（比目标小的最大结点）代替z，然后从BST中删除这个直接后继（或直接前驱）回到1或2的情况

```c++
// 二叉排序树的删除 采用右子树中最小元素代替策略
...
最好情况下，BST是完全二叉树，此时查找的时间复杂度为O(logn)
...
失衡树型也分为LL、RR、LR、RL四种

对应进行不同的旋转即可

只要把最靠近插入节点的失衡结点调整到正常，那么路径上的所有结点就都能平衡。



| 树型 |        判断条件        |                  调整办法                  |
| :--: | :--------------------: | :----------------------------------------: |
|  LL  | root左孩子的左子树导致 |               对root进行右旋               |
|  LR  | root左孩子的右子树导致 | 先对root的左孩子进行左旋，再对root进行右旋 |
|  RR  | root右孩子的右子树导致 |               对root进行左旋               |
|  RL  | root右孩子的左子树导致 | 先对root的右孩子进行右旋，再对root进行左旋 |
...
// 左旋
node* L(node *root) {
    node *t = root->r;	// 让root的右孩子t当根
    root->r = t->l;		// root作为t的左孩子
    t->l = root;		// root将t的左孩子当做右孩子
    return t;
}

// 右旋
node* R(node *root) {
    node *t = root->l;	// 让root的左孩子t当根
    root->l = t->r;		// root作为t的右孩子
    t->r = root;		// root将t的右孩子当左孩子
    return t;
}
...
AVL的建树是通过一个个结点插入逐步形成的，当新的结点插入时，可能会导致不平衡，此时就通过调整，然后再插入，直到所有结点都已经插入。

**手动调整的好方法：** 从插入结点出发，从下往上找第一个不平衡的结点，将该结点和路径上和他相连的三个结点拿出来调整，再将其他结点插入，回避了左旋右旋
...
删除操作可能也会导致不平衡，需要进行调整。

调整完后可能会根结点的父结点失去平衡……甚至回溯到根结点（导致树高减1）
...
查找的过程和BST相同，与给定值进行关键字比较的次数不超过树的高度

含n个结点的AVL最大高度为O(logn)，因此查找的时间复杂度O(logn)
...
可以求给定结点的AVL的查找所需的最多比较次数（树的最大高度）

例如：**在含12个结点的AVL中查找某个结点的最多比较次数（树的最大高度）是？** **5**

```c
公式 N_h=N_(h-1)+N_(h-2)+1 其中N_h表示高度为h的AVL中最少的结点数
...
1. 【878，2017】若二叉搜索树（排序树）是有 *N* 个结点的完美二叉树，则不正确的说法是 ==C==
...
2. 【878，2018】若二叉搜索树（排序树）是有 *N* 个结点的完全二叉树，则不正确的说法是 ==B==
...
3. 【2012】AVL的高度为6，所有非叶结点的平衡因子均为1，则总结点数为：==20==

    ```
    所有非叶结点的平衡因子均为1，也就是AVL最少结点数的情况
    递推公式为：
    T1 = 1， T2 = 2
    Tn = T（n-1） + T（n-2）+ 1
    
    高度为1时，节点数1；高度为2时，结点数2
    ```

    

4. 高度为5的AVL至少有 ==12== 个结点
...
1. **开放地址法** （也就是空地址对同义词和非同义词都开放）：`H新 = （H旧 + d） % m `
    - **线性探测法** ：顺序去找下一个空单元。  **聚集现象** 
    - **平方探测法** （也叫二次探测法）：`d = 0、1方、-1方、2方、-2方…` 避免了堆积，但只能探测一半单元
    - 再散列法：冲突的时候用另一个hash函数算增量d **最多m-1次就会遍历表中的全部位置**
    - 伪随机法
2. 链地址法（拉链法） 将所有同义词存储在一个线性链表中，适合于经常需要插入和删除的情况，删除可以直接删除结点，因为不会影响其他关键词

注意：

- 开址法删除不能直接删除，要打标记，且定期维护
- 目前来看，线性探测再散列就是线性探测法，二次探测再散列就是平方探测法
...
4. 【2018】现有长度为7、初始为空的散列表HT，散列函数H(K) = k % 7，用 **线性探测再散列法** 解决冲突。将关键字22、43、15依次插入HT后，ASL成功 =  ==2==
...
5. 【2019】现有长度为11且初始为空的散列表HT，散列函数为H(key) = key % 7，采用 **线性探测再散列法** 解决冲突。将87、40、30、6、11、22、98、20依次插入HT后，ASL失败 = ==6==

    ```
    先画表,H(key)=0~6
    ASL失败 = (9+8+7+6+5+4+3)/模 = (9+8+7+6+5+4+3)/7
    ```

    

6. 已知线性表的关键字集合 { 21，11， 13，25，48，6，39，83，30，96，108 }，散列函数为 h=key%11，采用 **分离链接法** 解决冲突。则成功查找的平均查找长度为： ==1.36==

     

7. 
    【878，2017】若用平方探测法解决冲突，则插入新元素时，以下陈述正确的是：==A==
...
8. 【878，2018】若N个关键词被散列映射到同一个单元，并且用 **分离链接法** 解决冲突 ，则找到这N个关键词所用的比较次数为：==N(N+1)/2==

    ```
    在同一个单元下组成链表结构，查找第一个元素比较1次，第二个元素比较2次……
    1+2+...+n
    ```

     

9. 【878，2020】给定散列表大小为 11，散列函数为 H(Key)=Key%11。若采用 **平方探测法** 处理冲突：hi(k)=(H(k)+i^2^)%11。将关键字序列 { 23, 2, 16, 32, 12 } 依次插入到散列表中。那么元素 12 存放在散列表中的位置是：==6==

    ![IMG_0495](assets/数据结构_笔记/IMG_0495.PNG)
...
1. 【重要】假设关键字为`{26, 36, 41, 38, 44, 15, 68, 12, 6, 51, 25}` ，采用 **链地址法** 解决冲突，设装填地址 `α=0.75` ，散列函数为 `H(key)=key%P`， 求等概率情况下成功和失败的ASL（失败只将和关键字的比较计算在内）

    ![7.5.5_应用题_1](assets/数据结构笔记/7.5.5_应用题_1.PNG)

    

1. 【878，2017】将元素序列 { 18，23，26，20，2，35，5，36，64，22 } 按顺序插入一个初始为空的、大小为 13 的散列表中。散列函数为： H(Key) = Key%13，采用 **线性探测法** 处理冲突。

    ![QQ截图20221216094213](assets/数据结构_笔记/QQ截图20221216094213.png)

    

1. 【2010】将关键字序列{7,8,30,11,18,9,14}散列到散列表中。散列表的存储空间是一个下标从0开始的一维数组，散列函数为H(key) = (key * 3) mod 7，处理冲突采用 **线性探测再散列法** ，要求装填因子为0.7

    1. 请画出所构造的散列表
    2. 分别计算在等概率的情况下，查找成功和查找失败的ASL

    ![7.5.5-应用题-3](assets/数据结构_笔记/7.5.5-应用题-3-16986684191062.PNG)
 ...
 **算法的稳定性:**  如果两个关键字相同的元素，在排序之后相对顺序不变，则称为 **稳定** ，否则就是 **不稳定** 
...
1. 对任意7个关键字进行基于比较的排序，至少需要进行 `13` 次关键字之间的两两比较
   ...
   ```
   任意n个关键字排序的比较次数至少为log2 (n!)
   ```

2. 【878，2016】输入 10^6^ 个只有一位数字的整数，可以用 *O*(*N*) 复杂度将其排序的算法是：`桶排序`

     

3. 【878，2016】采用递归方式对顺序表进行 **快速排序** ，下列关于递归次数的叙述中，正确的是 `A`
...
4. 设有100个元素的有序序列，如果用折半插入排序再插入一个元素,则最大比较次数是 `7`

    ```
    100可被2除多少次，log100上取整
...
5. 对N个记录进行归并排序，归并趟数的数量级是logn `正确`

6. 对N个记录进行简单选择排序，比较次数和移动次数分别为O(N2)和O(N) **正确**

7. 对N个记录进行快速排序，最坏的时间复杂度是O(N^2^) 正确

8. 当元素基本有序时，冒泡和直接插入排序都会较少 正确

9. 要从50个键值中找出最大的3个值，选择排序比堆排序快 错误

    ```
    找出元素最大，快速选择就是专门找最大，最小，遍历3遍就解决事情
    ```

    

10. 直接选择排序算法在最好情况下的时间复杂度为O(N) 错误

    ```
    直接选择算法，无论如何都需要遍历整个数组选择最大的，比来比去都要O(N^2^)
    ```
...
11. 若数据元素序列{ 11，12，13，7，8，9，23，4，5 }是采用下列排序方法之一得到的第二趟排序后的结果，则该排序算法只能是：
      ...
      - 冒泡和选择需要选择最大最小，两边都不是最大最小，直接排除
      - 插入排序符合11，12，13排序符合
      - 归并，看两路归并，13，7不符合

     

12. 【878，2020】输入 10^6 个 2 位数的、从大到小有序的整数，现需要将它们从小到大排序。就排序速度而言，以下哪组关系是正确的？ `D`
...
将一个待排序的元素按其关键字大小插入 **已经排好序的子序列** ，直到所有元素插入完成
...
循环 i 从 2 到 n（ **将A[1]视作已经完成排序的子序列** ）

1. 将A[i]暂存到A[0]
2. 循环已经完成排序的子序列的A[1~i-1]，找到合适的插入位置 k
3. 将A[k~i-1]中所有元素依次 **向后移动一位** ，将A[0]的值复制到A[k]
...
- 时间复杂度：总共循环了n-1趟，每趟都需要执行 **比较** 和 **移动** ，比较和移动次数和 **初始状态相关** 
  -  **最好情况下** ：表中元素 **已经有序** ， 此时每趟只需要比较一次而不需要移动， **时间复杂度O(n)** 
  -  **最坏情况下** ：表中元素逆序， **时间复杂度O(n^2)**  
  -  **平均情况下** ： 两重循环， **时间复杂度O(n^2)**  
- 稳定性：**稳定**
...
直接插入排序的 **查找插入位置** ，这一步 **从顺序查找改为折半查找** 
...
- 时间复杂度：减少了比较次数（更快的找到插入位置），元素移动次数没有改变。 **时间复杂度O(n^2)**  
- 稳定性： **稳定** 
...
可以说，增量序列是 `5, 3, 1` ，但看到增量序列 `1, 3, 5` 也不要惊慌，一个意思
...
- 时间效率：最坏情况下 **时间复杂度O(n^2)**  ，一般情况下和增量序列的函数有关
- 稳定性： **不稳定** 
- 适用性： **仅适合于用线性表的顺序存储** 
...
指的是根据两个元素的关键字值的大小对这两个元素的位置进行对换。
 ...
 **从前往后（或者从后往前）** 两两比较相邻元素的值，如果是逆序，就进行交换，这个过程称为 **一趟冒泡** 

一趟冒泡能使得 **将未排序序列中最大的（或者最小的）放到它的位置** ，已完成排序的元素不参与下一趟排序。
...
每次从待排序表 `a[l, r]` 中选择一个元素作为枢轴 `pivot` ，**一趟快速排序（也称为划分）结束后，pivot左边的元素都小于等于它，pivot右边的元素都大于等于它，pivot放到了它的最终位置上** 。
...
![第八章_快速排序的手动模拟](assets/数据结构_笔记/第八章_快速排序的手动模拟.PNG)
...
- 空间复杂度：和递归工作栈的深度成正比。
    ...
    - 平均情况：**O(nlogn)**
- 时间复杂度：
    - 运行时间和划分是否对称有关，平均情况下， **时间复杂度O(nlogn)**
    - 当表 **初始有序或者逆序** 时， 导致划分区间分配不平均， **最坏的时间复杂度O(n^2)**  

- 稳定性： **不稳定** ，例如右边有两个值相等的小于pivot的元素，交换到左边时，次序已经发生了变化

- 特点：不产生有序子序列，但是每趟排序后将pivot放到他的最终位置
...
提高算法的效率，可以通过尽量选取一个能将区间中分的pivot，例如可以 **选左边界元素、中间元素、右边界元素的中间值作为pivot**
...
1. 【2019】排序过程中，对尚未确定最终位置的所有元素进行一遍处理称为一趟。下列序列中，**不可能** 是快速排序第二趟结果的是：
    ...
    - 对两个子表都进行了一次划分才能算一趟，而一次划分必定能使得pivot到它的最终位置
    - 排序的最终结果是：2 5 12 16 28 32 60 70
    - 对于A来说，第一趟确定了73的位置，他是最右，所以第二趟只有一个区间，确定了28的位置，可以
    - 对于B来说，第一趟确定了2或者72的位置，第二趟也只有一个区间，确定了另一个的位置，可以
    - 对于C来说，他可以第一趟确定28，然后第二趟确定2和32，分别位于他的左右两个区间
    - 对于D来说，没有确定最左和最右的元素，那么他应该有3个元素确定了才对，不符合
    - 也就是说2趟快排能确定2或3个元素的位置
...
假设当前是第i趟排序，从未排序序列 `a[i…n-1]` 中选择关键字最小的元素与 `a[i]` 交换，每趟排序可以确定一个元素的最终位置，n-1趟后整个排序表有序
...
- 空间效率： **时间复杂度O(1)** ，算法原地工作

- 时间效率：
    - swap()函数会移动元素3次，因此最坏情况下元素会移动 `3(n-1)` 次，最好情况（表已经有序）下移动0次；

    - 但比较次数始终是 `n(n-1)/2` ， 因此  **时间复杂度O(n^2)**  
...
如果将一维数组视为一棵 **完全二叉树** ，满足任一非根结点的关键字都小于等于其双亲的结点值，此时树中最大元素是是树的根结点，此时称这个数据结构为 **最大堆** ，反过来可以定义 **最小堆** 
...
2. 交换 `a[1]` 和 `a[n]` （也就是将堆顶输出），此时将 `a[n]` 视为完成排序的序列，将 `a[1…n-1]` 调整为堆，再输出堆顶
3. 重复，直到堆中只剩一个元素为止，此时完成排序（最后实现从小到大排序）

堆排序从序列后方逐渐形成有序序列，直到完成排序
...
- 大循环是从最后一个有孩子的结点（n/2）开始，**向左调整** ，直到根结点
- 每次调整过程中，又是将以k为根结点的树 **向下调整** ， 使得该树为大根堆
...
![第八章_堆排序](assets/数据结构_笔记/第八章_堆排序.PNG)
...
- 空间效率：仅使用了常数个辅助单元， **空间复杂度O(1)** 
- 时间效率：建堆的时间是O(n)，然后有n-1次调整操作，每次调整的时间复杂度和树高有关：O(logn)，总体： **时间复杂度O(nlogn)** 
- 稳定性： **不稳定** 
...
1. 设线性表中每个元素有两个数据项k1和k2,现对线性表按一下规则进行排序：先看数据项k1，k1值小的元素在前，大的在后；在k1值相同的情况下，再看k2，k2值小的在前，大的在后。满足这种要求的排序方法是 
    ...
    - k1的优先级比k2更高，因此需要先排k2再排k1（因为排序是对整体进行的 不是说第二趟排序是组内进行的）
 ...
 **归并** 是将2个或者两个以上的有序表组合成一个新的有序表。基于比较
...
![IMG_0496](assets/数据结构_笔记/IMG_0496.PNG)
...
- 时间效率：每趟归并时间复杂度O(n)，（两个数组每次找最小的放进临时数组），总共需要 `log2n上取整` 次归并，因此 **时间复杂度O(nlogn)**
...
基数排序基于 **关键字各位的大小** 进行排序

基数排序的思想借助了 **多关键字排序** ，来对单关键字进行排序
...
![第七章_桶排序](assets/数据结构_笔记/第七章_桶排序.png)
...
1. 【2016】对10TB的数据文件进行排序，应使用的方法是 `归并排序`

    ```
    这个内存放不下 是外部排序了。外部排序多采用归并排序
    ```
...
- 归并排序平均和最坏情况下空间都是0n，是本章空间最差的排序算法
- 快排在最坏情况下空间也能On
...
3. 有一种简单的排序算法，称为 **计数排序** （CountSort）。对一个待排序的表进行排序，结果存到另一个表中。（保证没有重复元素）计数排序针对表中的每一条记录，扫描待排序的表一趟，假设统计出c个元素比当前元素小，那就将这个关键字放到c表的下标c的位置上
    ...
    3）简单选择更好。简单选择比较次数 `n(n-1)/2` 而且只需要n个辅助空间。计数排序的时间和空间都不如简单选择。
...
5. 【2021】已知某排序算法如下：
    ...
    该算法基于计数排序的思想，count[i]表示序列中比a[i]大的元素个数。例如 `count[1] = 3` 指的就是有3个元素比a[1]大
