...
1. （878，1）设最大堆的层序遍历结果为 { 95, 72, 86, 50, 65, 12, 23, 42 }。用线性时间复杂度的算法将该堆调整为最小堆。则该树的中序遍历结果为：

    ```
    72, 50, 42, 65, 12, 86, 23, 95
    ```

    最大堆就是个banana，调整为最小堆的时候当成普通二叉树即可。然后用上面的弗洛伊德建堆法，线性时间

2. （878，4）在一个有 2333 个元素的最小堆中，下列哪个下标不可能是唯一最大元的位置？
...
9. 【878，2020】给定散列表大小为 11，散列函数为 H(Key)=Key%11。若采用平方探测法处理冲突：hi(k)=(H(k)+i^2^)%11。将关键字序列 { 23, 2, 16, 32, 12 } 依次插入到散列表中。那么元素 12 存放在散列表中的位置是：**5**
