...
假设当前是第i趟排序，从a[i…n-1]中选择关键字最小的元素与 `a[i]` 交换，每趟排序可以确定一个元素的最终位置，n-1趟后整个排序表有序
...
如果将一维数组视为一棵 **完全二叉树** ，满足任一非根结点的关键字都小于等于其双亲的结点值，此时树中最大元素是是树的根结点，此时称这个数据结构为 **大顶堆** ，反过来可以定义 **小顶堆** 
