...
6. 已知线性表的关键字集合 { 21，11， 13，25，48，6，39，83，30，96，108 }，散列函数为 h=key%11，采用分离链接法解决冲突。则成功查找的平均查找长度为： **1.36**

7. 
    【878】若用平方探测法解决冲突，则插入新元素时，以下陈述正确的是：**A**
...
8. 【878，2018】若N个关键词被散列映射到同一个单元，并且用 **分离链接法解决冲突** ，则找到这N个关键词所用的比较次数为：N(N+1)/2

9. 【878，2020】给定散列表大小为 11，散列函数为 H(Key)=Key%11。若采用 **平方探测法** 处理冲突：hi(k)=(H(k)+i^2^)%11。将关键字序列 { 23, 2, 16, 32, 12 } 依次插入到散列表中。那么元素 12 存放在散列表中的位置是：**5**
