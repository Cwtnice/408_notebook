...
3. 【878，2016】采用递归方式对顺序表进行快速排序，下列关于递归次数的叙述中，正确的是 **A**
...
5. 对N个记录进行归并排序，归并趟数的数量级是log2n **正确**
...
- 空间复杂度：和递归工作栈的深度成正比。
    ...
    - 平均情况：**O(logn)**
- 时间复杂度：
    - 运行时间和划分是否对称有关，**时间复杂度O(nlogn)**
    - 当表 **初始有序或者逆序** 时， **最坏的时间复杂度O(n^2)**  
