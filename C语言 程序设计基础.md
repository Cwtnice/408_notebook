...
5. 如果有变量定义`char **pp[10];`，那么下面说法正确的是：**D**
...
20. 以下定义中，哪个p不是指针？ **都是指针**

      ```c++
      char **p; // 指向指向char的指针
      char (*p)[10]; // 是数组指针，这个指针是指向一个大小为10的字符数组 注意和下面这个的区别
      char *p[6];	// p是char** 类型的，p中的每个元素是char*类型的
      ```
