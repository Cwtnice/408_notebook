...
30. 【重要】下列那个用法是正确的： `D`
    ...
    D.*(*p).name='D';  // .的优先级大于*，所以这里等于a[0].name[0]='D';
    ```
...
1. 当k不为0时，在下列选项中能将k的值赋值给变量m、n的是 **B**

    ```c
    A. m=k=n	// n赋值给k 再赋值给m
    B. (m=k)&&(n=m)
    C. (m=k)||(n=k) // k将值赋给m后不会执行n=k了
    D. (n=m)&&(m=k)	// m赋给k,再k赋给n
    ```

    

2. 设 `int a=2,b=5,c=8;` 则 `c|=(~a&&b)^!(a&b)` 的值为 **8**
...
3. 以下能使puts(s)语句正确输出ABCDE字符串的是：**C**

    ```c
    A. char s[5] = {"ABCDE"}; puts(s);
    B. char s[5] = {'A', 'B', 'C', 'D', 'E'}; puts(s);
    C. char *s; s="ABCDE"; puts(s);
    D. char *s; scanf("%s", s); puts(s);	// char *s不是字符串不能用%s
    ```
...
4. 设 `int a = 5, b = 7` ，表达式 `(a=b=7, b=a!=7)` 的值是 **0**

    ```c
    // 逗号表达式 最后输出最后一个表达式的值
    // 先对b赋值为7，再赋值给a 然后将a!=7的值也就是0赋值给b 再输出b
...
5. 设 `int a = 1, b = 2, c = 3, d = 4, m = 2, n = 2` 执行表达式 `(m=a>b)&&(n=c>d)` 后n的值为：**2**
...
7. 下列函数strmcpy实现将字符串t的第m(0 <= m < strlen(t)) 个字符开始拷贝到s，请填充完整
...
10. 若变量a是int类型，并执行了语句 `a = 'A' + 1.6` 则正确的描述是 `a的值是字符'A'的ASCII值加上1`

    ```
    最后a的值是66 就是字符B的ASCII值
    ```
